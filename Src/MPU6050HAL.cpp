...
HAL_StatusTypeDef MPU6050_HAL::get_pry(double *angle_buf)
{
	HAL_StatusTypeDef result;
	if (gyro_first_call)
	{ // on first gyro call, get values from accel
	...
	else
	{
		...
		angle_buf[0] = pitch_internal
				+ (gyro_buf[0] * elapsed / gyro_scale_factor);
		angle_buf[1] = roll_internal
				+ (gyro_buf[1] * elapsed / gyro_scale_factor);
		angle_buf[2] = yaw_internal
				+ (gyro_buf[2] * elapsed / gyro_scale_factor);

		//compensate for IMU yaw
		angle_buf[0] += angle_buf[1]
				* sin(M_PI * (gyro_buf[2] * elapsed / gyro_scale_factor) / 180);
